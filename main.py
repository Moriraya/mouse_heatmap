...
def evaluate2():
    ...
    for df_name in dataframes:
        ...
        df['acceleration'] = df['velocity'].diff() / df['dt'].shift(-1)

        # Calculate direction (angle in radians)
        df['direction'] = np.arctan2(df['dy'], df['dx'])

        # Calculate rate of change of direction
        df['direction_change_rate'] = np.abs(df['direction'].diff()) / df['dt'].shift(-1)

        valid_accelerations = df['acceleration'].replace([np.inf, -np.inf], np.nan).dropna()
        valid_direction_change_rates = df['direction_change_rate'].replace([np.inf, -np.inf], np.nan).dropna()

        average_acceleration [df_name] = valid_accelerations.mean() if not valid_accelerations.empty else 0
        average_direction_change_rate [df_name] = valid_direction_change_rates.mean() if not valid_direction_change_rates.empty else 0
        print(f"{df_name} average_acceleration:")
        print(average_acceleration[df_name])
        print(f"{df_name} average_direction_change_rate:")
        print(average_direction_change_rate[df_name])
        print(df)
    ...
    for df_name in heatmap_data:
        axs[i].imshow(heatmap_data[df_name], cmap='coolwarm', interpolation='nearest')
        axs[i].set_title(f'{df_name} Av.acc: {average_acceleration[df_name]:.2f} Av.dir.rate: {average_direction_change_rate[df_name]:.2f}')
        axs[i].axis('off')
        i += 1
